...
- **注册用户（User/Blogger）：** 所有注册用户都是博主（内容创作者），拥有完整的系统权限，可以使用系统的所有功能：
  - 可以浏览内容、发表评论、提问、回答、点赞、收藏、关注其他用户
  - 可以发布和管理自己的文章、视频、问答
  - 可以查看自己内容的数据分析
  - 可以管理自己内容的评论、弹幕等
  - 可以访问内容管理后台，进行内容管理和数据分析
  - 可以查看自己的互动记录（点赞过的、评论过的内容）
  - **权限边界：** 所有注册用户权限完全平等，都可以使用系统的全部功能，但只能管理自己的内容，无权修改其他用户的内容。已发布内容的用户是有内容的博主，未发布内容的用户是空白博主，但功能权限无任何区别。
- **超级管理员（Super Admin）** ⏳ *（第三阶段：待开发）*
  - 拥有全站内容管理权限，可以对所有用户发布的内容进行审核、管理、删除等操作
  - 拥有用户管理权限，可以对用户进行管理、封禁等操作
  - 拥有系统设置权限，可以配置系统参数、管理分类标签等
...
**需求描述：** 用户可管理自己发布的所有文章，包括查看、编辑、删除、状态变更等。**注意：用户只能管理自己发布的内容，无权修改其他用户的内容。**
...
**需求描述：** 用户可浏览问题列表和详情，用户可管理自己相关的问题（自己提出的问题、自己发布的回答）。**注意：用户只能管理自己的内容，无权修改其他用户的问题和回答。**
...
**模块边界：** 独立模块，所有注册用户都可以访问和使用，用于管理自己的内容

#### 4.7.1 内容管理

**需求描述：** 所有注册用户都可以访问统一的仪表盘，集中管理自己发布的所有类型的内容。**注意：用户只能管理自己发布的内容，无权修改其他用户的内容。**
...
- **内容概览：**
  ...
  - 待处理事项（如有未回答的问题）
- **内容列表：**
  - 文章管理（复用4.2.2的功能）- 仅显示和管理自己发布的文章
  - 问答管理（复用4.3.4的功能）- 仅显示和管理自己提出的问题和发布的回答
  - 视频管理（第二阶段）- 仅显示和管理自己发布的视频
  - 统一的内容筛选和搜索（仅搜索自己的内容）
- **批量操作：**
  - 批量删除（仅限自己的内容）
  - 批量更改状态（仅限自己的内容）
