...
**问题描述：**
- 错误：`GET https://via.placeholder.com/150 net::ERR_NAME_NOT_RESOLVED`
- 数据库初始化脚本中使用了 `via.placeholder.com` 占位图片服务，但该服务无法访问
- 导致前端加载图片时出现网络错误，影响用户体验

**问题原因：**
- 数据库初始化数据中使用了外部占位图片服务 `via.placeholder.com`
- 该服务在某些网络环境下无法访问或已停止服务
- 所有用户头像和文章封面图都使用了该不可访问的 URL

**修复方案：**
- 将所有 `via.placeholder.com` 的图片 URL 替换为 `picsum.photos`（Lorem Picsum）服务
- 用户头像：`https://via.placeholder.com/150` → `https://picsum.photos/150/150?random={1-4}`
- 文章封面图：`https://via.placeholder.com/800x400` → `https://picsum.photos/800/400?random={1-5}`
- `picsum.photos` 是一个稳定的占位图服务，提供随机图片，可以正常访问

**修改文件：**
- `back/src/main/resources/db/crsa.sql`
